...
public class CardboardReticle : MonoBehaviour, ICardboardPointer {
  ...
  [SerializeField]
  Text helperText;
	public UnityEvent OGS;
  //Para events
	[System.Serializable]
	public class UE_OnTriggerUp : UnityEvent<GameObject>
	{
	}
	[SerializeField]
	public UE_OnTriggerUp OTD;

	[SerializeField]
	GameObject nfcHelper;
	Animator nfcHelperAnim;

	//PointShowData psdScript;
...
//		psdScript = GameObject.FindGameObjectWithTag ("PSD").GetComponent<PointShowData> ();
    materialComp = gameObject.GetComponent<Renderer>().material;
	
		nfcHelper = GameObject.FindGameObjectWithTag ("nfcHelper");
		if (nfcHelper != null) {
			nfcHelperAnim = nfcHelper.GetComponent<Animator> ();
			nfcHelper.SetActive (false);
		}
  ...
  void Update() {
    UpdateDiameters();
//		if (isHeldDown)
//			UE_OnTrigger.Invoke (targetedObj);
  }
  ...
  public void OnGazeStay(Camera camera, GameObject targetObj, Vector3 intersectionPosition) {
		targetedObj = targetObj;

		if (targetedObj != null && targetedObj.name == "root_mummy" ) {

			EgyptGameAnalyzer.sendLookedAtMummy ();
		}
		if (targetedObj != null && helperText!=null) {
			
			if (targetedObj.tag == "collectableObjects") {

				helperText.text = "Inspect Gem";
				if(nfcHelper!=null)
				nfcHelper.SetActive (true);
			}
			else if (targetedObj.name == "Battery") {
				helperText.text = "Use Batteries";
			} else if (targetedObj.name == "Brokenwires") {
				helperText.text = "Fix broken wires";
			} else if (targetedObj.name == "Switch") {
				helperText.text = "Turn on switch";
			} else if (targetedObj.tag == "collectableObject") {
				helperText.text = "Inspect";
			} else if (targetedObj.tag == "gate") {
				helperText.text = "It looks like its gonna give away soon";
...
//		if (targetedObj.tag != "Untaged") 
//		{
//			OGS.Invoke (targetObj);
//			//print (targetedObj.tag);
//		}
//	 if(targetObj!=null)
//			UE_OnGazeStay.Invoke (targetObj);
    SetGazeTarget(intersectionPosition);
  }
  ...
  ///
	public void OnGazeEnter(Camera camera, GameObject targetObj) {

	
	}
  /// The camera is the event camera and the target is the object the user
  /// previously looked at.
  public void OnGazeExit(Camera camera, GameObject targetObj) {
		targetedObj = null;	
		if(helperText!=null)
		helperText.text = "";
		if(nfcHelper!=null)
  ...
  public void OnGazeTriggerStart(Camera camera) 
    ...
    // Put your reticle trigger start logic here :)
		...
		//print(targetedObj);
//		if (targetObj.name != "Drop Object" || targetObj.name != "Use Object") 
//		{
			//OTD.Invoke (targetedObj);
//		}
	
//		if (targetedObj != null && targetedObj.layer != LayerMask.NameToLayer ("Ignore Raycast") && targetedObj.tag == "Objective") {
//			print (targetedObj.name);
//			targetedObj.GetComponent<Objective> ().updateState ();
//		}
		print("CLICKED");
		OGS.Invoke ();
		OTD.Invoke (targetedObj);
  ...
  private void SetGazeTarget(Vector3 target) {
	...
	void doubleClickMoniter()
	{
		...
		{
//			UE_OnDoubleTrigger.Invoke (targetObj);

		//	psdScript.GazeBasedInfo (targetObj);
