...
//	Text doubleText;
	...
	void walk ()
	{
		...
		Physics.Raycast (stickToGrndRay, out groundRayHitData, Mathf.Infinity, nonCollidableMask);
	...
	{
		avoidWalls ();
		if (groundRayHitData.point==Vector3.zero)
			this.transform.position = initPos;
		else
			transform.position = Vector3.Lerp(this.transform.position,new Vector3(transform.position.x,groundRayHitData.point.y+this.transform.localScale.y/1.5f, transform.position.z),0.4f);
	}
	...
	void avoidWalls()
	{
		...
		if( Physics.Raycast( avdWalls, out wallHit,10.5f,nonCollidableMask)&&(wallHit.distance <= stoppingDistance  ))
			isWalking = false;
		Debug.DrawRay (avdWalls.origin, avdWalls.direction);
	}
